...
#
# Uso: ./scripts/configurar-nginx-proxy.sh
# ============================================
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo -e "${YELLOW}Execute: sudo ./scripts/configurar-nginx-proxy.sh${NC}"
    exit 1
fi
...
# Verificar UFW
echo -e "${BLUE}🔒 Verificando UFW...${NC}"
ufw status

if [ $? -eq 0 ]; then
    echo -e "${YELLOW}💡 UFW está ativo. Verifique se as portas 80 e 443 estão liberadas:${NC}"
...
echo ""
echo -e "${YELLOW}2. Se ainda não conseguir acessar, verifique:${NC}"
echo -e "   - Se o Laravel está rodando: ${GREEN}ps aux | grep 'php8.2 artisan serve'${NC}"
echo -e "   - Se o Nginx está rodando: ${GREEN}systemctl status nginx${NC}"
echo -e "   - Se o PHP-FPM está rodando: ${GREEN}netstat -tlnp | grep :8082${NC}"
echo ""
echo -e "${YELLOW}3. Para configurar HTTPS (SSL):${NC}"
...
echo ""
echo -e "${YELLOW}4. Verificar logs do Nginx:${NC}"
